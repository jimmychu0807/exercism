...
mod concat {
	...
	fn list_of_lists() {
		let lists =
			vec![vec![1, 2], vec![3], vec![], vec![4, 5, 6]].into_iter().map(Vec::into_iter);
		let output = concat(lists);
	...
	fn list_of_nested_lists() {
		let lists = vec![vec![vec![1], vec![2]], vec![vec![3]], vec![vec![]], vec![vec![4, 5, 6]]]
			.into_iter()
			.map(Vec::into_iter);
		let output = concat(lists);
...
mod reverse {
	...
	fn list_of_lists_is_not_flattened() {
		let list = vec![vec![1, 2], vec![3], vec![], vec![4, 5, 6]].into_iter().map(Vec::into_iter);
		let output = reverse(list);

		let expected = vec![vec![4, 5, 6], vec![], vec![3], vec![1, 2]];

		assert_eq!(output.map(|subiter| subiter.collect::<Vec<_>>()).collect::<Vec<_>>(), expected);
	}
}
