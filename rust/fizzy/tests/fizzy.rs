...
fn nonsequential() {
	...
	let actual = fizz_buzz::<i32>().apply(collatz_12.iter().cloned()).collect::<Vec<_>>();
	let expected = vec!["fizz", "fizz", "fizz", "buzz", "buzz", "16", "8", "4", "2", "1"];
	assert_eq!(actual, expected);
}
...
fn minimal_generic_bounds() {
	...
	let actual = fizz_buzz::<Fizzable>()
		.apply(std::iter::successors(Some(Fizzable(1)), |prev| Some(*prev + 1.into())))
		.take(16)
		.collect::<Vec<_>>();
	let expected = [
		"1", "2", "fizz", "4", "buzz", "fizz", "7", "8", "fizz", "buzz", "11", "fizz", "13", "14",
