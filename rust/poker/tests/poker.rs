...
#[test]
fn card_and_sorting() {
	let input = "5S 7H 8D JC 4S 4D";
	let mut cards: Vec<_> =
		input.split(" ").map(|card_str| Card::try_from(card_str).unwrap()).collect();

	cards.sort();

	let expected = "4D 4S 5S 7H 8D JC";
	let expected_cards: Vec<_> =
		expected.split(" ").map(|card_str| Card::try_from(card_str).unwrap()).collect();
...
fn a_tie_has_multiple_winners() {
	...
	let output = winning_hands(input).into_iter().collect::<HashSet<_>>();
	let expected = ["3S 4S 5D 6H JH", "3H 4H 5C 6C JD"].into_iter().collect::<HashSet<_>>();
	assert_eq!(output, expected);
}
