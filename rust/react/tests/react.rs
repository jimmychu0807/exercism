...
fn compute_cells_can_depend_on_other_compute_cells() {
	...
	let output = reactor
		.create_compute(&[CellId::Compute(times_two), CellId::Compute(times_thirty)], |v| {
			v[0] + v[1]
		})
		.unwrap();
...
impl CallbackRecorder {
	fn new() -> Self {
		CallbackRecorder { value: std::cell::Cell::new(None) }
	}
...
fn error_adding_callback_to_nonexistent_cell() {
	...
	assert_eq!(Reactor::new().add_callback(output, |_: u32| println!("hi")), None);
}
...
fn adder_with_boolean_values() {
	...
	let a_xor_b =
		reactor.create_compute(&[CellId::Input(a), CellId::Input(b)], |v| v[0] ^ v[1]).unwrap();
	let sum = reactor
	...
	let carry_out = reactor
		.create_compute(&[CellId::Compute(a_xor_b_and_cin), CellId::Compute(a_and_b)], |v| {
			v[0] || v[1]
		})
		.unwrap();
