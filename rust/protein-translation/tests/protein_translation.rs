...
fn sequence_of_two_protein_codons_translates_into_proteins() {
	assert_eq!(translate("UUUUUU"), Some(vec!["Phenylalanine", "Phenylalanine"]),);
}
...
fn translate_rna_strand_into_correct_protein_list() {
	assert_eq!(translate("AUGUUUUGG"), Some(vec!["Methionine", "Phenylalanine", "Tryptophan"]),);
}
...
fn translation_stops_if_stop_codon_at_end_of_three_codon_sequence() {
	assert_eq!(translate("AUGUUUUAA"), Some(vec!["Methionine", "Phenylalanine"]),);
}
...
fn translation_stops_if_stop_codon_in_middle_of_six_codon_sequence() {
	assert_eq!(translate("UGGUGUUAUUAAUGGUUU"), Some(vec!["Tryptophan", "Cysteine", "Tyrosine"]),);
}
...
fn incomplete_rna_sequence_can_translate_if_valid_until_a_stop_codon() {
	assert_eq!(translate("UUCUUCUAAUGGU"), Some(vec!["Phenylalanine", "Phenylalanine"]),);
}
