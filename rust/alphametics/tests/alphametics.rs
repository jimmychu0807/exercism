...
fn puzzle_with_four_letters() {
	...
	let expected = [('A', 9), ('S', 2), ('M', 1), ('O', 0)].into_iter().collect();
	assert_eq!(answer, Some(expected));
}
...
fn puzzle_with_six_letters() {
	...
	let expected =
		[('N', 7), ('O', 4), ('T', 9), ('L', 1), ('A', 0), ('E', 2)].into_iter().collect();
	assert_eq!(answer, Some(expected));
}
...
fn puzzle_with_seven_letters() {
	...
	let expected = [('E', 4), ('G', 2), ('H', 5), ('I', 0), ('L', 1), ('S', 9), ('T', 7)]
		.into_iter()
		.collect();
	assert_eq!(answer, Some(expected));
}
...
fn puzzle_with_eight_letters() {
	...
	let expected = [('S', 9), ('E', 5), ('N', 6), ('D', 7), ('M', 1), ('O', 0), ('R', 8), ('Y', 2)]
		.into_iter()
		.collect();
	assert_eq!(answer, Some(expected));
}
